...
public class SqlLiteHelper extends SQLiteOpenHelper {
   ...
   * <p>"Goals" table has the following fields:</p>
   * <p>"id", primary id, autoincremented (starts at 1), "goal_total", text,
   * the name of the goal</p>
   */
  private static final String SQL_CREATE_GOALS_TABLE =
      "CREATE TABLE " + GoalsEntry.TABLE_NAME + "(" + GoalsEntry.COLUMN_NAME_ID
          + " INTEGER PRIMARY KEY AUTOINCREMENT, " + GoalsEntry.COLUMN_NAME_GOAL_TITLE + " TEXT );";

  /**
   * Command to create "QuadrantItems" table in Hexis database
   * <p>"QuadrantItems" table has the following fields:</p>
   * <p>"id", primary id, autoincremented (starts at 1)
   ...
   * "completion_status", integer, set to 0 for unfinished. Set to 1 for finished</p>
   */
